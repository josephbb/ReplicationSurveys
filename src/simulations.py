...
def simulate_reproducibility(row, i_data, prob_sig=96.0 / 99.0):
    ...
    signals = np.abs(np.random.normal(
        0, i_data['posterior']['tau'][chain, sample])).ravel()
    es = signals + \
        np.random.normal(0, i_data['posterior']
                         ['sigma'][chain, sample]).ravel()
    ...
    published = np.random.binomial(
        1, (prob_sig * significant) + (1 - prob_sig) * (1 - significant))
    ...
    rep_es = signals[published == 1] + \
        np.random.normal(0, np.array(
            i_data['posterior']['sigma'][chain, sample]).ravel()[published == 1])
