...
def simulate(i_data,
    ...
    return out
def simulate_reproducibility_alt(row, i_data, pubs=1000, prob_sig=96.0 / 99.0):
    count = row[0]
    N = row[1]
    J = row[2]
    alpha = row[3]
    ...
    for jidx in range(J):
        ...
        signals = np.abs(np.random.normal(0,
                            i_data['posterior']['tau'].values[chain, sample],
                            size=1000)).ravel()
        ...
        type_s = np.random.binomial(1,tt_ind_solve_power(
                                        effect_size=d_orig,
                                        alpha=alpha/2,
                                        alternative='lower',
                                        nobs1=np.int(N/2.0)))
        ...
        d_rep = np.random.normal(signals[published==1], sigma[published==1])
