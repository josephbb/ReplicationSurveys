...
def get_ci_es(df):
    with pm.Model() as m_2:
        ...
        mu = pm.Gamma('mu',1,1)
        ef = pm.Gamma('ef',mu=mu, sigma=sigma, observed=df['d_o'])
...
def plot_posterior_predictive(df, ppc, save_loc=False, dpi=300):
    ...
    ax.plot(df.direction*ppc['d_o'].mean(axis=0),
...
def plot_fig4a(samples_full, axs, max_effect=2,
    ...
    sns.kdeplot(np.abs(np.array(samples_full['tau']).ravel()), label=r'$\tau$', clip=[0, 10],
                shade=True, color=pal[0], ax=axs)
    sns.kdeplot(np.abs(np.array(samples_full['sigma']).ravel()), label=r'$\sigma$', clip=[0, 10],
...
def plot_sim_figure(i_data, sim_data, df,
                    ...
                    rep=35/97,
                    save_loc=False,
                    dpi=300, max_effect=2, res=100, pal="mako",k=35,n=97):
    """
...
def plot4a_alt(idata, max_effect):
    ...
    y = np.array([idata.posterior['a_sigma'].values  + \
              idata.posterior['b_sigma'].values*item for item in x])

    for idx, q in enumerate([3, 10, 25,50]):

        MAP = np.median(y.reshape(20, 4000),axis=1)
        PI = np.percentile(y.reshape(20, 4000), axis=1,q=[q/2, 100-q/2])
        plt.fill_between(x, PI[0], PI[1],color='grey', alpha=idx/5)
    plt.plot(x, MAP)
...
def plot_sim_figure_alt(i_data, sim_data, df,
    ...
    # Plot figuer a, set xlim
    plt.sca(axs[0][0])
    plot4a_alt(i_data, max_effect=3)
    axs[0][0].set_xlim(0, max_effect)
